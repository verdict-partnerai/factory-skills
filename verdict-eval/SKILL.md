...
description: |
  ...
  Operates entirely through GitHub Issue comments on 0xsmcai/factory-ops.
allowed-tools:
  - Bash
  - Read
  - Grep
...
All interaction happens via GitHub Issue comments on `0xsmcai/factory-ops`.

**Posting a question or comment:**
```bash
gh issue comment {number} -R 0xsmcai/factory-ops --body "[VERDICT] your message here"
```

**Reading the Issue and responses:**
```bash
gh issue view {number} -R 0xsmcai/factory-ops --comments
```

**Managing labels:**
```bash
gh issue edit {number} -R 0xsmcai/factory-ops --add-label "label-name"
gh issue edit {number} -R 0xsmcai/factory-ops --remove-label "label-name"
```
...
Issue: 0xsmcai/factory-ops#{number}
Status: DRAFT
Mode: Startup
...
```bash
# If approved:
gh issue edit {number} -R 0xsmcai/factory-ops --add-label "approved" --remove-label "in-review"

# If rejected:
gh issue edit {number} -R 0xsmcai/factory-ops --add-label "rejected" --remove-label "in-review"
```
...
```bash
gh issue comment {number} -R 0xsmcai/factory-ops --body "[VERDICT] Design doc committed: ideas/{project-name}/design-doc.md on branch design-doc/{project-name}"
```
